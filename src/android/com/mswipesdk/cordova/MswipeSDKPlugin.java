  ...
  public boolean execute(String action, JSONArray args,
    ...
    private void callAction() {
        ...
        try {
            ...
            else if (mAction.equals("printtext"))
            {
                printText(options);
            }
            else if (mAction.equals("printimage"))
            {
                printImage(options);
            }
        } catch (Exception e) {
    ...
    public void printText(final JSONObject options) throws Exception{

        if(IS_DEBUG_ON)
            Log.v("mswipesdkkk", "printText");
        ...
        try {
            ...
            String alignment = options.optString("align");
            int size = options.optInt("size");

            if(IS_DEBUG_ON)
                Log.v("mswipesdkkk", "text: " + text);

            if(IS_DEBUG_ON)
                Log.v("mswipesdkkk", "align: " + alignment);

            if(IS_DEBUG_ON)
                Log.v("mswipesdkkk", "size: " + size);


            FontLattice fontLattice = FontLattice.EIGHT;


            if(size == 2) {
                fontLattice = FontLattice.EIGHT;
            }
            else if(size == 3) {
                ...
                fontLattice = FontLattice.FORTY_EIGHT;
            }
            ...
            PrintRespCode printRespCode = Print.StartPrinting(text, fontLattice, true, align, true);



            if(IS_DEBUG_ON)
                Log.v("mswipesdkkk", "printRespCode: " + printRespCode);


            JSONObject obj = new JSONObject();

            boolean status =  false;
            String statusMessage  = "print success";

            if(printRespCode == PrintRespCode.Printer_Fail)
            {
                statusMessage  = "printer failed";
            }
            else if(printRespCode == PrintRespCode.Printer_PaperLack)
            {
                statusMessage  = "no paper";
            }
            else if(printRespCode == PrintRespCode.Printer_PaperTypeError)
            {
                statusMessage  = "paper type error";
            }
            else if(printRespCode == PrintRespCode.Printre_TooHot)
            {
                statusMessage  = "printer too hot";
            }
            else if(printRespCode == PrintRespCode.Printer_Busy)
            {
                statusMessage  = "printer busy";
            }
            else if(printRespCode == PrintRespCode.Printer_Bitmap_TooWide)
            {
                statusMessage  = "printer bitmap too wide";
            }
            else if(printRespCode == PrintRespCode.print_Unknow)
            {
                statusMessage  = "unknown error";
            }
            else if(printRespCode == PrintRespCode.Print_Success || printRespCode == PrintRespCode.Print_NO_CONTENT)
            {
                status =  true;
            }
            else
            ...
            if(IS_DEBUG_ON)
                Log.v("mswipesdkkk", "status: " + status);

            if(IS_DEBUG_ON)
                Log.v("mswipesdkkk", "statusMessage: " + statusMessage);
    ...
    public void printImage(final JSONObject options) throws Exception{

        if(IS_DEBUG_ON)
            Log.v("mswipesdkkk", "printImage");


        try {

            String baseimage = options.optString("bitmap");
            String alignment = options.optString("align");

            if(IS_DEBUG_ON)
                Log.v("mswipesdkkk", "Bitmap: "+ baseimage );
            ...
            if(IS_DEBUG_ON)
                Log.v("mswipesdkkk", "bytecount: " + icon.getByteCount());
            ...
            PrintRespCode printRespCode =  Print.StartPrintingImage(icon, Align.CENTER);
